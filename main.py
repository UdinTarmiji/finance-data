...
st.set_page_config(page_title="Finance Tracker", page_icon="💰") st.title("💰 Finance Tracker")
...
df = df.dropna(subset=["tanggal"]) df["tanggal"] = pd.to_datetime(df["tanggal"]) df = df.sort_values("tanggal") df["pemasukan"] = pd.to_numeric(df["pemasukan"], errors="coerce").fillna(0) df["pengeluaran"] = pd.to_numeric(df["pengeluaran"], errors="coerce").fillna(0) df["saldo"] = df["pemasukan"].cumsum() - df["pengeluaran"].cumsum()

--- Ringkasan ---

st.markdown("## 📊 Ringkasan") st.metric("💰 Total Saldo", f"Rp {df['saldo'].iloc[-1]:,.0f}") st.metric("📈 Total Pemasukan", f"Rp {df['pemasukan'].sum():,.0f}") st.metric("📉 Total Pengeluaran", f"Rp {df['pengeluaran'].sum():,.0f}")

--- Grafik Saldo ---

st.markdown("## 📅 Grafik Saldo") periode = st.selectbox("Pilih Periode", ["Harian", "Mingguan", "Bulanan", "Tahunan"]) chart_type = st.radio("Tipe Grafik", ["Line Chart", "Area Chart"])

resample_map = {"Harian": "D", "Mingguan": "W", "Bulanan": "M", "Tahunan": "Y"} df_chart = df.set_index("tanggal").resample(resample_map[periode]).sum(numeric_only=True) df_chart["saldo"] = df_chart["pemasukan"].cumsum() - df_chart["pengeluaran"].cumsum()

fig, ax = plt.subplots(figsize=(12, 5)) if chart_type == "Line Chart": ax.plot(df_chart.index, df_chart["saldo"], linewidth=3, color="blue") else: ax.fill_between(df_chart.index, df_chart["saldo"], color="skyblue", alpha=0.5) ax.plot(df_chart.index, df_chart["saldo"], linewidth=3, color="blue")

ax.set_ylabel("Saldo (Rp)") ax.set_title(f"Perkembangan Saldo - {periode}") ax.grid(True, linestyle="--", alpha=0.5) st.pyplot(fig)

--- Pie Chart Pengeluaran ---

st.markdown("## 🧁 Persentase Pengeluaran per Kategori") df_expense = df[df["pengeluaran"] > 0] if not df_expense.empty: kategori_data = df_expense.groupby("kategori")["pengeluaran"].sum() warna = ["#%06x" % random.randint(0, 0xFFFFFF) for _ in kategori_data] fig2, ax2 = plt.subplots() wedges, texts, autotexts = ax2.pie(kategori_data, labels=kategori_data.index, autopct="%1.1f%%", colors=warna, startangle=90) for autotext in autotexts: autotext.set_color("white") ax2.axis("equal") st.pyplot(fig2) st.markdown("### 💡 Detail Kategori") for kategori, nominal in kategori_data.items(): st.write(f"🔹 {kategori}: Rp {nominal:,.0f}")

--- History & Edit ---

with st.expander("📄 Lihat Riwayat Transaksi"): st.dataframe(df.sort_values("tanggal", ascending=False)) if st.button("✏️ Edit / Hapus Transaksi"): idx_list = list(df.index) selected_index = st.selectbox("Pilih Index", idx_list) selected_row = df.loc[selected_index]
